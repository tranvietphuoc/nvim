...
set GOPREFIX (brew --prefix golang)
set -gx GOROOT "$GOPREFIX/libexec"
set -gx PATH $PATH (go env GOROOT)/bin (go env GOPATH)/bin
...
set -gx PSQL_PAGER "pspg --csv --rr=2 --ignore-case -I --csv-header on -s 15 --bold-labels --bold-cursor"

# poetry Python
set POETRY_ROOT "$HOME/.poetry"
set -gx PATH "$POETRY_ROOT/bin" $PATH
